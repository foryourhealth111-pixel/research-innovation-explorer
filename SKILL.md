...
4. Run `scripts/build_idea_matrix.py` to generate `idea-matrix.csv`.
...
7. Review both `A -> B` and `B -> A`. Select a direction only when the sources support a clearer relationship.
8. In each review round, identify the single uncertainty most likely to change the recommendation. Perform one focused action, then update facts, inferences, status, confidence, and the next check.
9. Stop when further searching mainly repeats known information, a status is adequately supported, or the next decision requires researcher input. Preserve unresolved uncertainty in the record.
10. Produce a candidate landscape using `references/reporting-and-visualization.md` and `assets/templates/analysis-report-template.md`.
...
- Use `unknown`, `needs_check`, or `conflicting` whenever the evidence warrants them.
